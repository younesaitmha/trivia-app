...
class TriviaTestCase(unittest.TestCase):
    ...
    def test_404_paginated_questions(self):
        ...
        # get the valid number of pages and give a number out of range
        res = self.client().get(f'/api/v1.0/questions?page={len(questions)//10 + 10}')
        data = json.loads(res.data)
    ...
    def test_get_questions_by_category(self):
        ...
        response = self.client().get(f'/api/v1.0/categories/{category.id}/questions')
        data = json.loads(response.data)

        # test if this category has no questions the request with return error 404
        if len(questions) == 0:
    ...
    def test_404_get_questions_by_category(self):
        ''' test error 404 by passing in the request inexisting category id '''

        # query the last id in the table of categories
        id_not_in_db = Category.query.order_by(Category.id.desc()).first().id
        response = self.client().get(f'/api/v1.0/categories/{id_not_in_db+2}/questions')
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 404)
        self.assertEqual(data['success'], False)
        self.assertEqual(data['error'], 404)
    ...
    def test_delete_question(self):
        ...
        question_to_delete = Question.query.order_by(Question.id.desc()).first()
        res = self.client().delete(f'/api/v1.0/questions/{question_to_delete.id}')
        data = json.loads(res.data)
    ...
    def test_404_delete_question(self):
        ''' test error 404 in delete question by passing to the URI inexisting question's id '''

        # query the last id in the table of questions
        question_id_not_in_db = Question.query.order_by(Question.id.desc()).first().id
        res = self.client().delete(f'/api/v1.0/questions/{question_id_not_in_db+2}')
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 404)
        self.assertEqual(data['success'], False)
        self.assertEqual(data['error'], 404)
    ...
    def test_search_question(self):
        ''' test searching questions by inserting a question and search by this question '''
        ...
        response = self.client().post(
            '/api/v1.0/questions/search', json={'searchTerm' : question.question}
        )


        data = json.loads(response.data)

        # query the questions that has the inserted question in the question attribute
        search_results = Question.query.filter(Question.question.ilike(f'%{question.question}%')).all()
    ...
    def test_failed_play_quiz(self):
        ...
        response = self.client().post('/api/v1.0/quizzes', json={'pip8': 'null'})
        data = json.loads(response.data)
